...
const server = http.createServer((request, response) => {
  ...
  // Paso 3: Validamos la existencia de path para concatenar "static"
  if (path) {
    ...
    if (!error) {
      ...
      fs.readFile(fileSytemPath, (error, file) => {
        if (!error) {
          ...
          // Paso 5.2: Devolver un error si no se puede leer el archivo.
          status = 500;
      ...
      // Paso 4.1: Devolver un error de lectura del archivo
      status = 404;
